...
4. 钱包支付：用户进行支付，通常是通过对应的钱包进行的，大家可以回忆一下自己在购物中，支付的过程，不同的端，对钱包支付的处理是不太一样的：
   ...
   ![2](/img/digging-deeper/duplicate-pay/640-2.png)

   - WAP端：手机的网页站，WAP端的支付一般是直接拉起对应的钱包，如果拉起钱包失败，就跳转界面
   ![3](/img/digging-deeper/duplicate-pay/640-3.png)

   - PC端：PC端，通常是打开收银台，展示一个二维码，通过钱包扫码支付，下面是京东的微信支付扫码页
   ![1](/img/digging-deeper/duplicate-pay/640-1.png)

5. 支付回调：用户完成支付后，三方支付平台，会回调商户，通知支付结果。

6. 同步订单状态：支付服务在确认支付完成后，会向订单服务同步支付的结果，订单服务变更订单的状态，由未支付-》待发货，客户端通过轮询、长连接，或者服务端主动推送的方式，在界面上变更订单状态。

我们再从支付流水的角度看一下支付状态的变化：

![4](/img/digging-deeper/duplicate-pay/640-4.png)
...
![5](/img/digging-deeper/duplicate-pay/640-5.png)
...
![6](/img/digging-deeper/duplicate-pay/640-6.png)
...
![7](/img/digging-deeper/duplicate-pay/640-7.png)
...
![8](/img/digging-deeper/duplicate-pay/640-8.png)
...
![9](/img/digging-deeper/duplicate-pay/640-9.png)
...
![10](/img/digging-deeper/duplicate-pay/640-10.png)
...
1. 订单支付的简要流程
   ...
   6. 同步订单状态
2.  为什么订单会重复支付
   1.  用户重复点击支付，而支付接口未防重导致的重复支付
   2.  “掉单”导致的重复支付
      - 外部“掉单”：三方支付的支付状态没有同步或者没有及时同步到商城，这叫外部掉单
      - 内部“掉单”：支付服务的状态没有同步到订单，或者客户端没有及时获取到订单状态，这叫内部掉单。
   3. 多渠道导致的重复支付（微信，支付宝），
3.  如何防止订单重复支付
   1. 不管是“申请支付”、还是“支付回调”,都应该以订单维度加锁，防止并发下的重复操作
   2. 针对“多渠道导致的重复支付”导致的重复支付，用户重复支付的话，可以取消掉正在支付中的流水，再进行支付。
   3. 发起支付的时候，有流水正在支付中，如果第三方支付平台不支持取消支付的，在支付回调的时候，检查用户是否已经有成功流水，对后来的流水进行退款处理（但是我们不推荐这样的危险操作，评估下来，这样风险很大）。
   4. 针对外部“掉单”的，我们通过第三方支付平台提供的查询接口，主动轮询（定时任务，延时消息）
   5. 针对内部“掉单”的，我们通过同步调用+异步通知方式防止内部“掉单”
      1.  同步调用，一般调用3次，如果失败了会重试
      2.  异步通知，如果同步调用失败了，我们通过消息队列的重试机制，来尽可能保证支付状态的同步。
      3.  业务侧（订单）也可以主动查询订单是否支付成功
