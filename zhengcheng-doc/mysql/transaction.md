...
事务隔离性是锁来实现的，原子性、一致性、持久性通过数据库的 `redo log` 和 `undo log` 来完成的。 `redo log`称为重做日志，用来保证事务的原子性和持久性，`undo log`用来保证事务的一致性。

有人或许认为`redo`是`undo`的逆过程，其实不对的。`redo`和`undo`的作用都可以看做是一种恢复操作，`redo`恢复提交事务修改的页，而`undo`回滚行记录。因此两者记录的内容不同，`redo`通常是物理日志，记录的是**页的物理修改操作**，`undo`是逻辑日志，根据每行记录进行记录。
